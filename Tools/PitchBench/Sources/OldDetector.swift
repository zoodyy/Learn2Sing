...
/// Verbatim port of the app's PitchDetector analysis (as of commit ae59be0).
final class OldDetector: RealtimeDetector {
    var name: String { "old" }
    private(set) var _pitch: Double? = nil
    ...
    var analysedThisHop = false
    func process(_ channel: UnsafePointer<Float>, _ n: Int) -> Bool {
        analysedThisHop = false
        processInner(channel: channel, n: n, sampleRate: sampleRate)
        return analysedThisHop
    }
    var current: Double? { _pitch }
    ...
    private func processInner(channel: UnsafePointer<Float>, n: Int, sampleRate: Double) {
        ...
        analysedThisHop = true; analyze(sampleRate: sampleRate, minFreq: minFreq, maxFreq: maxFreq, maxLag: maxLag)
    }
    ...
    private func publish(_ value: Double?) { _pitch = value }
}
